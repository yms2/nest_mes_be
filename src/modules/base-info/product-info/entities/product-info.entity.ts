...
export class ProductInfo extends BaseEntity{
  ...
  @Column({name: 'product_size', type:'varchar', length:50, comment: '규격'})
  productSize: string;

  @ApiProperty({example:'kg', description: '발주단위'})
  @Column({name: 'product_order_unit', type:'varchar', length: 20, comment: '발주단위'})
  productOrderUnit: string;

  @ApiProperty({example:'ea', description: '재고단위'})
  @Column({name: 'product_inventory_unit', type:'varchar', length: 20, comment: '재고단위'})
  productInventoryUnit: string;

  @ApiProperty({example:'14', description:'수량 당 수량'})
  @Column({name: 'unit_quantity',type: 'varchar', length:10, comment: '수량 당 수량'})
  unitQuantity: string;

  @ApiProperty({example:20, description:'안전재고'})
  @Column({name: 'safe_inventory', type: 'varchar', length:10, comment: '안전재고'})
  safeInventory: string;

  @ApiProperty({example:20, description:'과세구분'})
  @Column({name: 'tax_type', type: 'varchar', length:10, comment: '과세구분'})
  taxType: string;

  @ApiProperty({example:20, description:'단가'})
  @Column({name: 'price', type: 'varchar', length: 10, comment: '단가'})
  productPrice: string;

  @ApiProperty({example:'비고내용', description:'비고'})
  @Column({name: 'product_bigo', type: 'varchar', length: 100, comment: '비고'})
  productBigo: string;
