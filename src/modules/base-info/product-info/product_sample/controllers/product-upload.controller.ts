...
import { ProductUploadService, ValidationResponse, UploadResponse } from '../services/product-upload/product-upload.service';

@ApiTags('ProductUpload')
@Controller('product-upload')
export class ProductUploadController {
  constructor(private readonly uploadService: ProductUploadService) {}

  @Post('validate')
  @Auth()
  @ApiOperation({ summary: '품목 정보 엑셀 검증' })
  ...
  @Post('confirmed')
  @Auth()
  @ApiOperation({ summary: '검증된 품목 정보 저장' })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
