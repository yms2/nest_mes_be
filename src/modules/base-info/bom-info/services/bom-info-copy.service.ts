...
export class BomCopyService {
  ...
  async copyBom(sourceProductCode: string, targetProductCode: string): Promise<{ message: string }> {
    // 소스 BOM 조회
    const sourceBoms = await this.bomRepository.find({ where: { parentProductCode: sourceProductCode } });
    ...
    // 타겟 BOM 생성
    const newBoms = sourceBoms.map(bom => this.bomRepository.create({
